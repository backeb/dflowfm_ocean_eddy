...
## Setup Balltree using ds.FlowElem_xcc,_yzw as reference dataset
## Use Haversine calculate distance between points on the earth from lat/long
## haversine - https://pypi.org/project/haversine/ 
## ds.FlowElem_xcc,_yzw are the lon lat points where we HAVE ucx and ucy
#tree = BallTree(np.deg2rad(np.swapaxes([ds.FlowElem_xcc.data, ds.mesh2d_face_y.data],0,1)), metric='haversine')
#
...
#    if (np.diff(gds.FlowElem_xcc.data[gds.mesh2d_edge_faces.data[i,:].astype(int)-1]) == 0): # check y-coords to see if the first one is bigger/smaller then adjust velocities
#        # we want to find ds.FlowElem_xcc and ds.FlowElem_ycc that are closest to gds.mesh2d_edge_x.data[i] and gds.mesh2d_edge_y.data[i]
...
#    elif (np.diff(gds.FlowElem_xcc.data[gds.mesh2d_edge_faces.data[i,:].astype(int)-1]) != 0):
#        distances, indices = tree.query(np.deg2rad(np.c_[gds.mesh2d_edge_x.data[i], gds.mesh2d_edge_y.data[i]]), k = 3)
